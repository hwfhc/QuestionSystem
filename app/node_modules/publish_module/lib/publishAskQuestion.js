...
function publishAskQuestion(title,description,score,authorID,callback){

    let config = this.config;

    let isTitleDuplicate = new Promise((resolve,reject) => {
        let sql = 'SELECT ID FROM AskQuestionTable WHERE title="' + title + '"';
    ...
    function titleNotDuplicate(){
        ...
        config.modules['saferman'].sql(sql,function(results){
            ...
            function initAskQuestionTable(NextUsableID){
                let sql = 'INSERT INTO AskQuestionTable (ID,title,description,score,time,authorID) VALUE (' +
                    NextUsableID + ",'" +
                    title + "','" +
                    description + "'," +
                    score + ",0," +
